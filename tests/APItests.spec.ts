...
const productListAPIURL = "https://automationexercise.com/api/productsList";
const lastProductId = 43;
...
let response = await request.get(productListAPIURL);
...
const response = await request.get(productListAPIURL);
...
test('Check last product in product list API', async ({request}) => {
    ...
    const data:productListResponse = await response.json();
    expect(data.responseCode).toBe(200);
    expect(response.url()).toBe(productListAPIURL);
    
    const lastProduct = data.products[data.products.length-1];
    console.log(lastProduct);
    expect(lastProduct.id).toBe(lastProductId);
